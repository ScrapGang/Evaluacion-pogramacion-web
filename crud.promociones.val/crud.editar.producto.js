...
const expresionespdte = {
	...
	descuentopdte: /^[$]\d{1,1000000000}$/, 
	fechainipdte: /^\d\d[/]\d\d[/]\d\d\d\d$/, 
	fechatermpdte: /^\d\d[/]\d\d[/]\d\d\d\d$/,  
	preciofinpdte: /^[$]\d{1,1000000000}$/  
}
...
const validarFormulariopdte = (e) => {
	switch (e.target.name) {
		...
		break;
		case "descuentopdte":
			validarCampopdte(expresionespdte.descuentopdte, e.target, 'descuentopdte');
		break;
		case "fechainipdte":
			validarCampopdte(expresionespdte.fechainipdte, e.target, 'fechainipdte');
		break;
		case "fechatermpdte":
			validarCampopdte(expresionespdte.fechatermpdte, e.target, 'fechatermpdte');
		break;
		case "preciofinpdte":
			validarCampopdte(expresionespdte.preciofinpdte, e.target, 'preciofinpdte');
		break;
	}
}
...
formulariopdte.addEventListener('submit', (e) => {
	...
	if(campospdte.idpdte && campospdte.nombrepdte && campospdte.marcapdte && campospdte.stockpdte && campospdte.preciopdte && campospdte.descuentopdte && campospdte.fechainipdte && campospdte.fechatermpdte && campospdte.preciofinpdte ){
