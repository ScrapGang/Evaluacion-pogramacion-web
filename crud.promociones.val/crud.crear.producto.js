...
const expresionespdt = {
	...
	descuentopdt: /^[$]\d{1,1000000000}$/, 
	fechainipdt: /^\d\d[/]\d\d[/]\d\d\d\d$/, 
	fechatermpdt: /^\d\d[/]\d\d[/]\d\d\d\d$/,  
	preciofinpdt: /^[$]\d{1,1000000000}$/ 
}
...
const validarFormulariopdt = (e) => {
	switch (e.target.name) {
		...
		break;
		case "descuentopdt":
			validarCampopdt(expresionespdt.descuentopdt, e.target, 'descuentopdt');
		break;
		case "fechainipdt":
			validarCampopdt(expresionespdt.fechainipdt, e.target, 'fechainipdt');
		break;
		case "fechatermpdt":
			validarCampopdt(expresionespdt.fechatermpdt, e.target, 'fechatermpdt');
		break;
		case "preciofinpdt":
			validarCampopdt(expresionespdt.preciofinpdt, e.target, 'preciofinpdt');
		break;
	}
}
...
formulariopdt.addEventListener('submit', (e) => {
	...
	if(campospdt.idpdt && campospdt.nombrepdt && campospdt.marcapdt && campospdt.stockpdt && campospdt.preciopdt && campospdt.descuentopdt && campospdt.fechainipdt && campospdt.fechatermpdt && campospdt.preciofinpdt ){
		formulariopdt.reset();
